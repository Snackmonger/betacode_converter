...
def render_betacode_vowel(radical: str, coefficients: str) -> str:
    ...
    # Quick and dirty function for now, but this needs to be cleaned up
    # and reworked a bit for brevity.
    row: int = 0
    col: int = 0
    offset: int = 0
    ...
    if coefficients in ACUTE_SYMBOL + GRAVE_SYMBOL:
        if radical.islower():
            ...
            offset = get_greek_vowel_simple_accent_offset(radical)
            col += offset
    ...
    # N.B. diaeresis does not have unaccented version.
    ...
    # N.B. Subscript without breathing has accent characters in
    # lowercase, but not upper (thus len(coefficients) ==1)
